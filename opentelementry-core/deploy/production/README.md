...
opentelementry-deploy provision <EC2_PUBLIC_IP>

# Or if already NixOS, just deploy updates:
opentelementry-deploy deploy <EC2_PUBLIC_IP>
```

**Nix Deploy Commands:**

```bash
opentelementry-deploy provision <ip>  # Fresh instance: install NixOS + deploy
opentelementry-deploy deploy <ip>     # Update existing NixOS instance
opentelementry-deploy status <ip>     # Check container status
opentelementry-deploy logs <ip>       # View logs
opentelementry-deploy certs <ip>      # Setup Let's Encrypt
opentelementry-deploy ssh <ip>        # SSH into instance
```
...
**Go (opentelementry-go):**

```go
opentelementryOpts := options.OpentelementryOptions{
    Telemetry: options.DefaultTelemetry(),
}
opentelementryOpts.Telemetry.OTLP.Enabled = true
opentelementryOpts.Telemetry.OTLP.Host = "otel.yourdomain.com"
opentelementryOpts.Telemetry.OTLP.Port = 443
opentelementryOpts.Telemetry.OTLP.Secure = true
opentelementryOpts.Telemetry.OTLP.UseHTTP = true
...
sudo docker stop opentelementry-prod-envoy-1
...
# Copy certificates
sudo cp /etc/letsencrypt/live/telemetry.yourdomain.com/fullchain.pem /opt/opentelementry/certs/
sudo cp /etc/letsencrypt/live/telemetry.yourdomain.com/privkey.pem /opt/opentelementry/certs/

# Restart Envoy
sudo docker start opentelementry-prod-envoy-1
```
...
```bash
ssh ec2-user@<IP> "sudo docker logs opentelementry-prod-otelcol-1"
ssh ec2-user@<IP> "sudo docker logs opentelementry-prod-grafana-1"
```

### Restart services

```bash
ssh ec2-user@<IP> "cd /opt/opentelementry && sudo docker-compose restart"
```
