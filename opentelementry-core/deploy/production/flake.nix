{
  description = "Opentelementry Telemetry - Production Deployment";
  ...
  outputs = { self, nixpkgs, flake-utils }:
    let
      ...
      nixosConfigurations = {
        ...
        opentelementry-ec2 = nixpkgs.lib.nixosSystem {
          system = "x86_64-linux";
          modules = [
            "${nixpkgs}/nixos/modules/virtualisation/amazon-image.nix"
            ./nix/modules/opentelementry-telemetry.nix
            ({ config, pkgs, ... }: {
              ec2.hvm = true;
              system.stateVersion = "24.05";

              services.opentelementry-telemetry.enable = true;
    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Deploy script
        deployScript = pkgs.writeShellScriptBin "opentelementry-deploy" ''
          set -e
          ...
          usage() {
            echo "Opentelementry Telemetry NixOS Deployment"
            echo ""
            echo "Usage: opentelementry-deploy <command> <ip-address>"
            echo ""
            echo "Commands:"
          ...
          deploy() {
            ...
            # Copy module
            ${pkgs.openssh}/bin/ssh -i "$SSH_KEY" "root@$ip" "mkdir -p /etc/nixos/modules /var/lib/opentelementry/config"
            ${pkgs.openssh}/bin/scp -i "$SSH_KEY" -r ./nix/modules/* "root@$ip:/etc/nixos/modules/"
            ${pkgs.openssh}/bin/scp -i "$SSH_KEY" -r ./config/* "root@$ip:/var/lib/opentelementry/config/"
          ...
          EOF
            ...
            # Create secrets
            echo "$GRAFANA_PASS" | ${pkgs.openssh}/bin/ssh -i "$SSH_KEY" "root@$ip" "cat > /var/lib/opentelementry/grafana-password"
            echo "$OTLP_AUTH_TOKEN" | ${pkgs.openssh}/bin/ssh -i "$SSH_KEY" "root@$ip" "cat > /var/lib/opentelementry/otlp-token"
          ...
          certs() {
            ...
            ${pkgs.openssh}/bin/ssh -i "$SSH_KEY" "root@$1" "
              ...
              cp /etc/letsencrypt/live/$DOMAIN/*.pem /var/lib/opentelementry/certs/
      ...
      in {
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "🚀 Opentelementry Telemetry Development Environment"
            echo ""
        ...
        devShells.docker = pkgs.mkShell {
          name = "opentelementry-docker";
          buildInputs = commonDeps ++ dockerDeps;
          shellHook = ''
            echo "🐳 Opentelementry Telemetry (Docker only)"
            export PATH="$PWD/scripts:$PATH"
          '';
        };
        ...
        devShells.aws = pkgs.mkShell {
          name = "opentelementry-aws";
          buildInputs = commonDeps ++ awsDeps ++ dockerDeps ++ [ deployScript ];
          shellHook = ''
            echo "☁️  Opentelementry Telemetry (AWS)"
            export PATH="$PWD/scripts:$PATH"
          '';
        };
        ...
        apps.default = {
          type = "app";
          program = "${deployScript}/bin/opentelementry-deploy";
        };
