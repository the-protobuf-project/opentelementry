...
let
  cfg = config.services.opentelementry-telemetry;
...
in {
  options.services.opentelementry-telemetry = {
    enable = mkEnableOption "Opentelementry Telemetry observability stack";
    ...
    dataDir = mkOption {
      type = types.path;
      default = "/var/lib/opentelementry";
  ...
  config = mkIf cfg.enable {
    ...
    # Create network
    systemd.services.opentelementry-network = {
      description = "Create Opentelementry Telemetry network";
      ...
      serviceConfig = {
        ...
        ExecStart = "${pkgs.podman}/bin/podman network create opentelementry-net || true";
        ExecStop = "${pkgs.podman}/bin/podman network rm opentelementry-net || true";
      };
    };
    ...
    virtualisation.oci-containers.containers.loki = {
      image = images.loki;
      autoStart = true;
      extraOptions = [ "--network=opentelementry-net" ];
      volumes = [
