...
echo "=== Opentelementry Telemetry EC2 Deployment ==="
...
ssh -i "$SSH_KEY" ec2-user@"$PUBLIC_IP" "sudo mkdir -p /opt/opentelementry/{config,envoy,dashboards,docker,certs} && sudo chown -R ec2-user:ec2-user /opt/opentelementry"
...
ssh -i "$SSH_KEY" ec2-user@"$PUBLIC_IP" "
    ...
    chmod 644 /opt/opentelementry/certs/privkey.pem
"
...
ssh -i "$SSH_KEY" ec2-user@"$PUBLIC_IP" "cat > /opt/opentelementry/.env << 'EOF'
...
ssh -i "$SSH_KEY" ec2-user@"$PUBLIC_IP" "cd /opt/opentelementry && sudo docker-compose -f docker-compose.prod.yaml up -d --build"
