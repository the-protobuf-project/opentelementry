#!/bin/bash
# Opentelementry Telemetry - Deployment Script
...
if [ ! -f "$SSH_AUTH_DIR/id_ed25519" ]; then
    ...
    ssh-keygen -t ed25519 -f "$SSH_AUTH_DIR/id_ed25519" -N "" -C "opentelementry-deploy"
...
usage() {
    cat << EOF
${BLUE}Opentelementry Telemetry Deployment${NC}
...
copy_files() {
    local ip=$1
    echo "Copying files..."
    ssh -i "$SSH_KEY" "ec2-user@$ip" "sudo mkdir -p /opt/opentelementry/{config,envoy,dashboards} && sudo chown -R ec2-user:ec2-user /opt/opentelementry"
    scp -i "$SSH_KEY" -r "$PROD_DIR/config/"* "ec2-user@$ip:/opt/opentelementry/config/"
    scp -i "$SSH_KEY" -r "$PROD_DIR/envoy/"* "ec2-user@$ip:/opt/opentelementry/envoy/"
    scp -i "$SSH_KEY" "$PROD_DIR/docker-compose.prod.yaml" "ec2-user@$ip:/opt/opentelementry/"
    [ -d "$OTEL_DIR/dashboards" ] && scp -i "$SSH_KEY" -r "$OTEL_DIR/dashboards/"* "ec2-user@$ip:/opt/opentelementry/dashboards/" 2>/dev/null || true
}

create_env_file() {
    local ip=$1
    ssh -i "$SSH_KEY" "ec2-user@$ip" "cat > /opt/opentelementry/.env << EOF
...
start_services() {
    ...
    ssh -i "$SSH_KEY" "ec2-user@$ip" "cd /opt/opentelementry && sudo docker-compose -f docker-compose.prod.yaml up -d --build"
    sleep 10
...
cmd_logs() {
    ...
    [ -z "$ip" ] && { usage; exit 1; }
    [ -n "$svc" ] && ssh -i "$SSH_KEY" "ec2-user@$ip" "sudo docker logs -f $svc" || ssh -i "$SSH_KEY" "ec2-user@$ip" "cd /opt/opentelementry && sudo docker-compose -f docker-compose.prod.yaml logs -f"
}
