# EC2-only deployment for Opentelementry Telemetry
...
# Security group
resource "aws_security_group" "opentelementry" {
  name        = "opentelementry-telemetry-sg"
  description = "Opentelementry telemetry security group"
...
# SSH Key
resource "aws_key_pair" "opentelementry" {
  key_name   = "opentelementry-telemetry-key"
...
locals {
  user_data = <<-EOF
    ...
    # Create opentelementry directory structure
    mkdir -p /opt/opentelementry/{config,certs,envoy,dashboards}

    # Generate self-signed certs (replace with Let's Encrypt later)
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
      -keyout /opt/opentelementry/certs/privkey.pem \
      -out /opt/opentelementry/certs/fullchain.pem \
      -subj "/CN=${var.domain_name}"

    # Set permissions
    chown -R ec2-user:ec2-user /opt/opentelementry
...
resource "aws_spot_instance_request" "opentelementry" {
  ami                    = data.aws_ami.al2023.id
  instance_type          = var.instance_type
  key_name               = aws_key_pair.opentelementry.key_name
  vpc_security_group_ids = [aws_security_group.opentelementry.id]
...
resource "aws_ec2_tag" "opentelementry_name" {
  resource_id = aws_spot_instance_request.opentelementry.spot_instance_id
  key         = "Name"
  value       = "opentelementry-telemetry"
}

# Elastic IP for stable DNS pointing
resource "aws_eip" "opentelementry" {
  domain = "vpc"
...
resource "aws_eip_association" "opentelementry" {
  instance_id   = aws_spot_instance_request.opentelementry.spot_instance_id
  allocation_id = aws_eip.opentelementry.id
}

# Get instance details for public DNS
data "aws_instance" "opentelementry" {
  instance_id = aws_spot_instance_request.opentelementry.spot_instance_id
  depends_on  = [aws_eip_association.opentelementry]
}
...
output "public_ip" {
  ...
  value       = aws_eip.opentelementry.public_ip
}

output "ssh_command" {
  description = "SSH into the instance"
  value       = "ssh ec2-user@${aws_eip.opentelementry.public_ip}"
}

output "deploy_command" {
  description = "Run this after terraform apply to deploy services"
  value       = "cd .. && ./scripts/deploy-ec2.sh ${aws_eip.opentelementry.public_ip}"
}
...
output "dns_instructions" {
  ...
  value       = "Create an A record: ${var.domain_name} -> ${aws_eip.opentelementry.public_ip}"
}
...
resource "aws_acm_certificate" "opentelementry" {
...
resource "aws_route53_record" "acm_validation" {
  for_each = var.route53_zone_id != "" ? {
    for dvo in aws_acm_certificate.opentelementry[0].domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "opentelementry" {
  count    = var.route53_zone_id != "" ? 1 : 0
  provider = aws.us_east_1

  certificate_arn         = aws_acm_certificate.opentelementry[0].arn
...
resource "aws_cloudfront_distribution" "telemetry" {
  ...
  comment             = "Opentelementry Telemetry Dashboard"
  default_root_object = ""
  ...
  viewer_certificate {
    acm_certificate_arn      = aws_acm_certificate_validation.opentelementry[0].certificate_arn
  ...
  depends_on = [aws_acm_certificate_validation.opentelementry]
}
...
resource "aws_cloudfront_distribution" "otel" {
  ...
  enabled         = true
  is_ipv6_enabled = true
  comment         = "Opentelementry OTLP Ingestion Endpoint"
...
resource "aws_route53_record" "origin" {
  ...
  type    = "A"
  ttl     = 300
  records = [aws_eip.opentelementry.public_ip]
}
...
output "acm_certificate_arn" {
  ...
  value       = var.route53_zone_id != "" ? aws_acm_certificate.opentelementry[0].arn : "N/A - set route53_zone_id to enable"
}

output "acm_certificate_status" {
  description = "ACM certificate validation status"
  value       = var.route53_zone_id != "" ? aws_acm_certificate_validation.opentelementry[0].id : "N/A"
}
...
output "telemetry_url" {
  ...
  value       = var.route53_zone_id != "" ? "https://${var.domain_name}" : "https://${aws_eip.opentelementry.public_ip} (self-signed cert)"
}

output "otel_endpoint" {
  description = "OTLP HTTP endpoint for sending telemetry"
  value       = var.route53_zone_id != "" ? "https://${var.otel_domain_name}/v1/traces" : "http://${aws_eip.opentelementry.public_ip}:4318/v1/traces"
}
