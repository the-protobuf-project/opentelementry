...
int main() {
    ...
    auto opentelementry = opentelementry::Opentelementry::builder("chat-service", "1.0.0")
    ...
    OPENTELEMENTRY_LOG_INFO("Chat service started");
    OPENTELEMENTRY_LOG_DEBUG("Debug mode enabled");
    OPENTELEMENTRY_LOG_INFO("OpenTelemetry logging example");

    int active_rooms = 3;
    int total_users = 42;
    std::string init_msg = "Service initialized with " + std::to_string(active_rooms) +
                           " active rooms and " + std::to_string(total_users) + " users";
    OPENTELEMENTRY_LOG_INFO(init_msg.c_str());
    ...
    OPENTELEMENTRY_LOG_INFO(user_msg.c_str());
    OPENTELEMENTRY_LOG_INFO_DATA("User message received", msg1);
    OPENTELEMENTRY_LOG_DEBUG_DATA("Processing message from user", msg1);
    ...
    OPENTELEMENTRY_LOG_INFO(voice_msg.c_str());
    OPENTELEMENTRY_LOG_INFO_DATA("Voice message received", msg2);
    ...
    OPENTELEMENTRY_LOG_WARN(rate_msg.c_str());
    OPENTELEMENTRY_LOG_WARN_DATA("Rate limit approaching", msg3);

    int error_code = 500;
    std::string error_msg = "Failed to process message (error code: " +
                            std::to_string(error_code) + ") for user " + msg3.user_id;
    OPENTELEMENTRY_LOG_ERROR(error_msg.c_str());
    OPENTELEMENTRY_LOG_ERROR_DATA("Failed to process message", msg3);
    ...
    OPENTELEMENTRY_LOG_INFO(stats_msg.c_str());
    OPENTELEMENTRY_LOG_INFO("Chat service shutting down");
