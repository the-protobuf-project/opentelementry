...
McapWriter::McapWriter(const ServiceOptions& service_opts, const std::string& path)
    ...
    options.library = "opentelementry-cpp";
    writer_.open(file_, options);

    is_open_ = true;
    register_schemas();

    log_channel_id_ = ensure_channel("/opentelementry/logs", "Log");
    metric_channel_id_ = ensure_channel("/opentelementry/metrics", "Metric");
    span_channel_id_ = ensure_channel("/opentelementry/spans", "Span");
#endif
}
