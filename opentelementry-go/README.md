...
Install the Opentelementry SDK using Go modules:
...
Create a `opentelementry.toml` in your project root (auto-discovered):
...
func main() {
    // Automatically loads opentelementry.toml
    p, err := opentelementry.New().Build()
    if err != nil {
        panic(err)
    }
    defer p.Close()
...
Opentelementry provides a unified observability stack built on OpenTelemetry standards:

```mermaid
graph TB
    App[Your Application]
    Opentelementry[Opentelementry SDK]

    subgraph "Opentelementry Components"
        Logger[Logger]
        Metrics[Metrics]
        Tracer[Tracer]
        Profiler[Profiler]
    ...
    App --> Opentelementry
    Opentelementry --> Logger
    Opentelementry --> Metrics
    Opentelementry --> Tracer
    Opentelementry --> Profiler
    Opentelementry --> MCAP
...
Opentelementry provides structured logging with automatic context propagation and
...
```go
func handleRequest(ctx context.Context, p *opentelementry.Opentelementry) {
...
p, err := opentelementry.New().
    ...
    WithLogLevel(opentelementry.ModuleLevel_3). // Debug — full observability
    Build()
```
...
Opentelementry supports OpenTelemetry metrics including counters, gauges, and histograms.
...
```go
p, err := opentelementry.New().WithService("job-worker", "1.0.0").WithMCAP("./metrics.mcap").Build()
if err != nil {
    panic(err)
}
defer p.Close()
...
Opentelementry provides automatic distributed tracing with OpenTelemetry, enabling you
...
type OrderRequest struct {
    OrderID   string  `opentelementry:"trace:order.id"`
    UserID    string  `opentelementry:"trace:user.id"`
    Amount    float64 `opentelementry:"trace:order.amount"`
    Currency  string  `opentelementry:"trace:order.currency"`
}

func processOrder(
    ctx context.Context, p *opentelementry.Opentelementry, req OrderRequest,
) error {
    return p.Tracing.Trace(
        ctx, "ProcessOrder", req,
        func(ctx context.Context, span *opentelementry.Span) error {
...
```go
func handleCheckout(
    ctx context.Context, p *opentelementry.Opentelementry,
) error {
    return p.Tracing.Trace(
        ctx, "Checkout", nil,
        func(ctx context.Context, span *opentelementry.Span) error {
...
func validateCart(ctx context.Context, p *opentelementry.Opentelementry) (context.Context, error) {
...
Opentelementry supports multiple configuration methods with automatic discovery.

### Configuration File Formats

Opentelementry auto-discovers config files in this order:
...
Environment variables override config file values. Use `OPENTELEMENTRY_` prefix:

```bash
export OPENTELEMENTRY_TELEMETRY_OTLP_ENDPOINT=otel-collector:4317
...
p, err := opentelementry.New().
    ...
    WithConfig("./config/opentelementry.toml").
...
type ConversationRequest struct {
    ...
    UserInput string `opentelementry:"trace:input.text"`
    UserID    string `opentelementry:"trace:user.id"`
}
...
func processConversation(
    ctx context.Context, p *opentelementry.Opentelementry, req ConversationRequest,
) error {
    return p.Tracing.Trace(
...
func processInput(
    ctx context.Context, p *opentelementry.Opentelementry, input string,
) (string, error) {
...
func retrieveContext(
    ctx context.Context, p *opentelementry.Opentelementry, userID string,
) ([]string, error) {
...
func generateResponse(
    ctx context.Context, p *opentelementry.Opentelementry, input string, context []string,
) (string, error) {
...
```go
opts := options.OpentelementryOptions{}
