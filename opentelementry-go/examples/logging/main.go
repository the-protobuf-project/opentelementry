...
type ChatMessage struct {
	MessageID string `json:"message_id" opentelementry:"attribute:message.id"`
	...
	Type      string `json:"type" opentelementry:"attribute:message.type"` // text, speech, llm_response
...
type TranscriptionEvent struct {
	TranscriptionID string  `json:"transcription_id" opentelementry:"attribute:transcription.id"`
	UserID          string  `json:"user_id" opentelementry:"attribute:user.id"`
	RoomID          string  `json:"room_id" opentelementry:"attribute:room.id"`
	Language        string  `json:"language" opentelementry:"attribute:transcription.language"`
	Status          string  `json:"status" opentelementry:"attribute:transcription.status"` // success, failed
	Duration        float64 `json:"duration_ms" opentelementry:"attribute:transcription.duration"`
...
type LLMRequestEvent struct {
	RequestID    string `json:"request_id" opentelementry:"attribute:llm.request_id"`
	UserID       string `json:"user_id" opentelementry:"attribute:user.id"`
	RoomID       string `json:"room_id" opentelementry:"attribute:room.id"`
	Model        string `json:"model" opentelementry:"attribute:llm.model"`
	Status       string `json:"status" opentelementry:"attribute:llm.status"` // success, failed
	TokensUsed   int    `json:"tokens_used" opentelementry:"attribute:llm.tokens"`
	ResponseTime int64  `json:"response_time_ms" opentelementry:"attribute:llm.response_time"`
...
func main() {
	...
	defer func() {
		if err := p.Close(); err != nil {
			fmt.Printf("Error closing opentelementry: %v\n", err)
		}
	}()
