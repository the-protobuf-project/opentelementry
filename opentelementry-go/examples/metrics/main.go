...
type LLMMetrics struct {
	TokensProcessed int64   `opentelementry:"metric:counter:llm.tokens.processed"`
	ResponseTime    float64 `opentelementry:"metric:histogram:llm.response.time"`
	ActiveRequests  int64   `opentelementry:"metric:gauge:llm.requests.active"`
	CacheHitRate    float64 `opentelementry:"metric:gauge:llm.cache.hit_rate"`
}

// TranscriptionMetrics for speech-to-text
type TranscriptionMetrics struct {
	AudioDuration float64 `opentelementry:"metric:histogram:transcription.audio.duration"`
	Confidence    float64 `opentelementry:"metric:gauge:transcription.confidence"`
	WordCount     int64   `opentelementry:"metric:counter:transcription.words.count"`
}
	...
	for i := 0; i < 120; i++ {
		...
		if err := p.Metrics.Record(llmMetrics,
			opentelementry.WithAttributes(
				opentelementry.StringAttribute("model", "gpt-4"),
				opentelementry.StringAttribute("user_type", "premium"),
			),
		); err != nil {
		...
		if i%3 == 0 {
			...
			if err := p.Metrics.Record(transMetrics,
				opentelementry.WithAttributes(
					opentelementry.StringAttribute("language", "en"),
					opentelementry.StringAttribute("audio_format", "wav"),
				),
			); err != nil {
