...
func main() {
	...
	nats, err := opentelementry.New().
		WithService("nats-module", "0.2.0").
		WithLogLevel(opentelementry.ModuleLevel_3). // Code says Level3 (Debug)
		Build()
	if err != nil {
		panic(err)
	}
	...
	vision, err := opentelementry.New().
		WithService("vision-module", "0.1.0").
		WithLogLevel(opentelementry.ModuleLevel_3). // Code says Level3, no TOML override → Level3 wins
		Build()
	if err != nil {
		panic(err)
	}
	...
	fmt.Println("  env var (OPENTELEMENTRY_LOGGING_MODULES_<NAME>_LEVEL)")
