...
import (
	...
	"github.com/the-protobuf-project/opentelementry/opentelementry-go/internal/logging"
	"github.com/the-protobuf-project/opentelementry/opentelementry-go/internal/metrics"
	"github.com/the-protobuf-project/opentelementry/opentelementry-go/internal/profiling"
	"github.com/the-protobuf-project/opentelementry/opentelementry-go/internal/telemetry"
	"github.com/the-protobuf-project/opentelementry/opentelementry-go/internal/tracing"
...
// LogLevel is a type alias for options.LogLevel so modules can use opentelementry.Level1, etc.
...
// Opentelementry is the main framework struct that provides access to all telemetry services.
// It supports both the legacy logging interface and the new unified OpenTelemetry-based telemetry.
type Opentelementry struct {
...
// Builder provides a fluent API for configuring and creating a Opentelementry instance.
type Builder struct {
	serviceOpts        options.ServiceOptions
	opentelementryOpts options.OpentelementryOptions
	configPath         string
	err                error
}

// New creates a new Opentelementry Builder with sensible defaults.
// Use the builder methods to configure, then call Build() to create the Opentelementry instance.
//
// Example:
//
...
func (b *Builder) WithConfig(configPath string) *Builder {
	...
	opentelementryOpts, serviceOpts, err := options.LoadConfigWithDefaults(configPath)
	if err != nil {
		b.err = err
		return b
	}
	b.opentelementryOpts = *opentelementryOpts
...
// via [logging.modules.<service-name>] or env vars (OPENTELEMENTRY_LOGGING_MODULES_<NAME>_LEVEL).
//
...
//	    WithLogLevel(opentelementry.ModuleLevel_3). // "I'm a level 3 module"
//	    Build()
func (b *Builder) WithLogLevel(level LogLevel) *Builder {
	...
	if b.opentelementryOpts.Logging.Modules == nil {
		b.opentelementryOpts.Logging.Modules = make(map[string]options.ModuleOptions)
	}
	// Only set if config didn't already provide a per-module override for this service
	if _, exists := b.opentelementryOpts.Logging.Modules[b.serviceOpts.Name]; !exists {
		b.opentelementryOpts.Logging.Modules[b.serviceOpts.Name] = options.ModuleOptions{
			Level: level,
		}
	}
	return b
...
func (b *Builder) WithOTLP(host string, port int) *Builder {
	...
	b.opentelementryOpts.Telemetry.OTLP.Host = host //nolint:staticcheck // Deprecated but kept for backward compatibility
	b.opentelementryOpts.Telemetry.OTLP.Port = port
	b.opentelementryOpts.Telemetry.OTLP.Enabled = true

	// Auto-configure based on host
	b.opentelementryOpts.Telemetry.OTLP.Secure = !isLocalHost(host)
	return b
}
...
func (b *Builder) WithOTLPHeaders(headers map[string]string) *Builder {
	...
	b.opentelementryOpts.Telemetry.OTLP.Headers = headers
	return b
}
...
func (b *Builder) WithMCAP(path string) *Builder {
	...
	b.opentelementryOpts.Foxglove.Enabled = true
	b.opentelementryOpts.Foxglove.McapPath = path
	return b
}
...
func (b *Builder) WithProfiling(serverAddress string) *Builder {
	...
	b.opentelementryOpts.Profiling.Enabled = true
	b.opentelementryOpts.Profiling.ServerAddress = serverAddress
	return b
}
...
func (b *Builder) WithTracing() *Builder {
	...
	b.opentelementryOpts.Tracing.Enabled = true
	b.opentelementryOpts.Telemetry.Tracing.Enabled = true
	return b
}
...
// Build creates the Opentelementry instance with the configured options.
func (b *Builder) Build() (*Opentelementry, error) {
	if b.err != nil {
		return nil, b.err
	}

	// Auto-configure OTLP settings based on host
	autoConfigureOTLP(&b.opentelementryOpts.Telemetry.OTLP)

	ctx := context.Background()

	// Initialize unified telemetry service
	tel, err := telemetry.New(ctx, b.serviceOpts, b.opentelementryOpts.Telemetry)
	if err != nil {
		return nil, err
	}
	...
	if b.opentelementryOpts.Foxglove.Enabled && b.opentelementryOpts.Foxglove.McapPath != "" {
		unifiedMcap, err = foxglove.NewUnifiedMcapWriter(b.serviceOpts, b.opentelementryOpts.Foxglove)
		if err != nil {
			return nil, err
		}
	}
	...
	p := &Opentelementry{
		...
		Logger:      logging.NewLogger(b.serviceOpts, b.opentelementryOpts.Logging, unifiedMcap, tel.GetLogger()),
		Metrics:     metrics.NewMetrics(b.serviceOpts, unifiedMcap, tel.GetMetrics()),
		Tracing:     tracing.NewTracing(b.serviceOpts, b.opentelementryOpts.Tracing, unifiedMcap, tel.GetTracer()),
		Profiler:    profiling.NewProfiler(b.serviceOpts, b.opentelementryOpts.Profiling, unifiedMcap),
	}
...
// NewLegacy creates a new Opentelementry instance using the legacy API (for backward compatibility).
// Deprecated: Use New().WithConfig().Build() instead.
func NewLegacy(ctx context.Context, serviceOpts options.ServiceOptions, opts options.OpentelementryOptions) (*Opentelementry, error) {
...
func (p *Opentelementry) Close() error {
