...
This module provides custom formatting functions for the Opentelementry logging system,
...
by inspecting the call stack and skipping _private Opentelementry frames.
...
def get_caller_info():
    ...
    Inspects the call stack to find the first frame outside the Opentelementry SDK,
    ...
    # Walk up the stack to find the first frame outside opentelementry module
    for frame_info in inspect.stack()[
        2:
    ]:  # Skip this method and the calling log method
        filename = frame_info.filename

        # Skip frames from opentelementry package itself
        if (
            "/opentelementry-py/opentelementry/" in filename
            or filename.endswith("/opentelementry/logging.py")
            or filename.endswith("/opentelementry/opentelementry.py")
        ):
            continue
