"""Dynaconf-based configuration management for Opentelementry SDK.

Configuration Priority (lowest to highest):
1. Default values (lowest priority)
2. Config file (opentelementry.toml / opentelementry.yaml / opentelementry.json)
3. Environment variables (.env file or OPENTELEMENTRY_* env vars)
4. Code-based (builder methods) - highest priority

Environment variables use OPENTELEMENTRY_ prefix with double underscores for nesting:
...
Single underscores are also accepted and translated automatically:
    ...
    OPENTELEMENTRY_TELEMETRY_OTLP_ENDPOINT=otel.example.com
...
def _find_config_files() -> list[str]:
    ...
    # Check for opentelementry.toml, opentelementry.yaml, opentelementry.json in current directory
    for ext in ["toml", "yaml", "yml", "json"]:
        path = Path.cwd() / f"opentelementry.{ext}"
        if path.exists():
    ...
    if not config_files:
        for ext in ["toml", "yaml", "yml", "json"]:
            path = Path.cwd() / ".config" / f"opentelementry.{ext}"
            if path.exists():
                config_files.append(str(path))
                break
...
def _translate_single_underscore_env_vars(prefix: str, validators: list) -> None:
    """Translate single-underscore OPENTELEMENTRY_ env vars to double-underscore form.

    Dynaconf uses __ as nesting delimiter. This allows callers to set
    OPENTELEMENTRY_TELEMETRY_OTLP_ENDPOINT instead of OPENTELEMENTRY_TELEMETRY__OTLP__ENDPOINT.
...
_translate_single_underscore_env_vars("OPENTELEMENTRY", _VALIDATORS)

# Initialize Dynaconf settings
settings = Dynaconf(
    envvar_prefix="OPENTELEMENTRY",
    envar_separator="_",
