"""Configuration options for Opentelementry SDK.

This module defines all configuration dataclasses and options for the Opentelementry SDK,
...
@dataclass
class OpentelementryOptions:
    """Main Opentelementry SDK configuration.

    Top-level configuration for the Opentelementry SDK, aggregating all subsystem options.
...
def from_config(
...
) -> tuple[ServiceOptions, OpentelementryOptions]:
    """Load Opentelementry configuration from config file and environment variables.
    ...
    Args:
        config_path: Optional path to config file. If not provided,
                    auto-discovers opentelementry.toml/yaml/json.

    Returns:
        A tuple containing (ServiceOptions, OpentelementryOptions) loaded from config.

    Example:
        # Auto-discover opentelementry.toml
        service_opts, opentelementry_opts = from_config()

        # Or specify config path
        service_opts, opentelementry_opts = from_config("./config/opentelementry.toml")
    """
    ...
    opentelementry_opts = OpentelementryOptions(
        telemetry=telemetry_opts,
        foxglove=foxglove_opts,
    )

    return service_opts, opentelementry_opts


# Keep from_env as alias for backward compatibility
def from_env() -> tuple[ServiceOptions, OpentelementryOptions]:
    """Load Opentelementry configuration from environment variables (deprecated).
