...
//! - Environment variables with `OPENTELEMENTRY_` prefix
...
use crate::options::{
    ...
    OpentelementryOptions, ProfilingOptions, ServiceOptions, TelemetryOptions, TracingOptions,
};

/// Complete Opentelementry configuration loaded from files/environment.
///
...
#[serde(default)]
pub struct OpentelementryConfig {
...
impl OpentelementryConfig {
    /// Creates a Figment instance for loading configuration.
    ///
    /// Configuration is loaded in this priority order (highest to lowest):
    /// 1. Environment variables (`OPENTELEMENTRY_*`)
    ...
    pub fn figment_with_path(config_path: Option<&str>) -> Figment {
        let mut figment = Figment::from(Serialized::defaults(OpentelementryConfig::default()));

        // Try to load from config file
        if let Some(path) = config_path {
            figment = Self::merge_config_file(figment, path);
        } else if let Ok(path) = std::env::var("OPENTELEMENTRY_CONFIG_PATH") {
            figment = Self::merge_config_file(figment, &path);
        } else {
        ...
        figment.merge(Env::prefixed("OPENTELEMENTRY_").split("_"))
    }
    ...
    /// Convert to ServiceOptions for Opentelementry initialization.
    ...
    /// Convert to OpentelementryOptions for Opentelementry initialization.
    pub fn to_opentelementry_options(&self) -> OpentelementryOptions {
...
mod tests {
    ...
    fn test_default_config() {
        let config = OpentelementryConfig::default();
    ...
    fn test_parse_endpoint() {
        let (host, port) = OpentelementryConfig::parse_endpoint("localhost:4317");
        assert_eq!(host, "localhost");
        assert_eq!(port, 4317);

        let (host, port) = OpentelementryConfig::parse_endpoint("http://otel-collector:4317");
        assert_eq!(host, "otel-collector");
        assert_eq!(port, 4317);

        let (host, port) = OpentelementryConfig::parse_endpoint("otel-collector");
        assert_eq!(host, "otel-collector");
        assert_eq!(port, 4317);
    }
    ...
    fn test_to_service_options() {
        let mut config = OpentelementryConfig::default();
