//! Opentelementry - Unified observability library for robotics and distributed systems.
//!
//! Opentelementry provides integrated logging, metrics, and tracing with support for
...
//!
//! ```no_run
//! use opentelementry::Opentelementry;
//! use opentelementry::logger;
//!
//! let _opentelementry = Opentelementry::builder("my-service", "1.0.0")
//!     .with_mcap("output.mcap")
//!     .build()
//!     .unwrap();
//!
//! logger::info!("Application started");
//! // Resources are automatically cleaned up when opentelementry goes out of scope
//! ```
...
pub use config::OpentelementryConfig;
...
/// Main Opentelementry instance that manages all observability components.
///
...
pub struct Opentelementry {
    pub logger: Logger,
    pub tracing: Option<tracing::OpentelementryTracing>,
    pub metrics: Metrics,
...
impl Opentelementry {
    ...
    ///
    /// This is the recommended way to initialize Opentelementry. Configuration is loaded from:
    /// 1. `OPENTELEMENTRY_CONFIG_PATH` environment variable
    /// 2. `opentelementry.toml` in current directory
    /// 3. `opentelementry.yaml` / `opentelementry.yml` / `opentelementry.json`
    /// 4. `.config/opentelementry.toml` / `.config/opentelementry.yaml` / `.config/opentelementry.json`
    ///
    /// # Examples
    ///
    /// ```no_run
    ...
    pub fn new() -> OpentelementryBuilder {
        OpentelementryBuilder::from_config()
    }

    /// Creates a new builder for configuring Opentelementry.
    ///
    /// # Arguments
    ///
    /// * `name` - Service name
    ...
    ///
    /// ```no_run
    /// use opentelementry::{Opentelementry, Environment};
    ///
    /// let opentelementry = Opentelementry::builder("my-service", "1.0.0")
    ///     .environment(Environment::Production)
    ///     .with_otlp("localhost", 4317)
    ///     .build()
    ///     .unwrap();
    /// ```
    pub fn builder(name: impl Into<String>, version: impl Into<String>) -> OpentelementryBuilder {
        OpentelementryBuilder::new(name, version)
    }

    /// Creates a new Opentelementry instance from options (legacy API).
    ///
    /// Initializes all observability components based on the provided configuration.
    /// For new code, prefer using `Opentelementry::new()` or `Opentelementry::builder()` instead.
    ///
    /// # Arguments
    ///
    /// * `service_opts` - Service configuration (name, version, environment)
    /// * `opentelementry_opts` - Opentelementry configuration (telemetry, Foxglove options)
    ///
    /// # Examples
    ///
    /// ```no_run
    /// use opentelementry::{Opentelementry, options::{ServiceOptions, OpentelementryOptions}};
    ///
    /// let service_opts = ServiceOptions::new("my-service", "1.0.0");
    /// let opentelementry_opts = OpentelementryOptions::new();
    /// let opentelementry = Opentelementry::init(service_opts, opentelementry_opts).unwrap();
    /// ```
    pub fn init(
        service_opts: options::ServiceOptions,
        opentelementry_opts: options::OpentelementryOptions,
    ) -> Result<Self> {
        let formatter = logging::OpentelementryFormatter::new();
        ...
        if opentelementry_opts.logging.level.is_set() {
            default_level = opentelementry_opts.logging.level.to_level_filter();
        }

        // Per-module override for this service (highest priority)
        if let Some(mod_opts) = opentelementry_opts.logging.modules.get(&service_opts.name)
        ...
        let mcap_writer = if opentelementry_opts.foxglove.enabled
            && !opentelementry_opts.foxglove.mcap_path.is_empty()
        {
            let writer = foxglove::UnifiedMcapWriter::new(
                &service_opts,
                &opentelementry_opts.foxglove.mcap_path,
            )?;
        ...
        let telemetry = Some(telemetry::TelemetryProvider::new(
            &service_opts,
            &opentelementry_opts.telemetry,
        )?);
        let otel_logger = telemetry
            .as_ref()
            .and_then(|t| t.get_logger("opentelementry"));
        ...
        // used by Opentelementry telemetry, so shutdown/flush is centrally managed.
        if let Some(tracer) = tracing_tracer.clone() {
            tracing::init_tokio_tracing(tracer)?;
        }

        // Initialize OpentelementryTracing for manual span management.
        let tracing_instance = Some(tracing::OpentelementryTracing::new(tracing_tracer));
    ...
    /// Manually closes the Opentelementry instance and cleans up resources.
    ///
    /// This shuts down telemetry providers and closes MCAP writers.
    ///
    /// **Note**: Resources are automatically cleaned up when Opentelementry goes out of scope
    ...
    /// let service_opts = ServiceOptions::new("my-service", "1.0.0");
    /// let opentelementry_opts = OpentelementryOptions::new();
    /// let mut opentelementry = Opentelementry::init(service_opts, opentelementry_opts).unwrap();
    ///
    /// // Use opentelementry...
    ///
    /// // Optional: Manually close if you need error handling
    /// opentelementry.close().unwrap();
    ///
    /// // Otherwise, resources are cleaned up automatically when opentelementry goes out of scope
    /// ```
...
impl Drop for Opentelementry {
    fn drop(&mut self) {
        let _ = self.close();
    }
}

/// Builder for configuring and creating a Opentelementry instance.
///
...
///     .unwrap();
/// ```
pub struct OpentelementryBuilder {
    name: Option<String>,
...
impl OpentelementryBuilder {
    ...
    /// OTLP to local collector on [`DEFAULT_OTEL_COLLECTOR_OTLP_PORT`] (gRPC), same as [`opentelementry_local_otel!`].
    ...
    /// Builds and initializes the Opentelementry instance.
    pub fn build(self) -> Result<Opentelementry> {
        // Load config from file if available
        let config = if let Some(path) = &self.config_path {
            config::OpentelementryConfig::load_from(path).ok()
        } else {
            config::OpentelementryConfig::load().ok()
        };

        // Start with config values or defaults
        let (mut service_opts, mut opentelementry_opts) = if let Some(cfg) = &config {
            (cfg.to_service_options(), cfg.to_opentelementry_options())
        } else {
            ...
            (
                ...
                options::OpentelementryOptions::new(),
            )
        };
        ...
        if self.service_from_code {
            let name = self
                .name
                .clone()
                .unwrap_or_else(|| "opentelementry-service".to_string());
        ...
        if let (Some(host), Some(port)) = (self.otlp_host.clone(), self.otlp_port) {
            opentelementry_opts.telemetry.enabled = true;
            opentelementry_opts.telemetry.otlp.enabled = true;
            opentelementry_opts.telemetry.otlp.endpoint = format!("{}:{}", host, port);
            opentelementry_opts.telemetry.otlp.host = host;
            opentelementry_opts.telemetry.otlp.port = port;
        }

        if let Some(token) = self.otlp_auth_token {
            opentelementry_opts.telemetry.otlp.auth_token = Some(token);
        }

        if !self.otlp_headers.is_empty() {
            opentelementry_opts.telemetry.otlp.headers = self.otlp_headers;
        }

        if let Some(secure) = self.otlp_secure {
            opentelementry_opts.telemetry.otlp.secure = secure;
        }
        ...
        // Tracing export: match opentelementry-go — app tracing + OTLP must not disagree
        ...
        if let Some(path) = self.mcap_path {
            ...
            opentelementry_opts.foxglove.mcap_path = path;
        }

        Opentelementry::init(service_opts, opentelementry_opts)
    }
}
