...
pub fn init_tokio_tracing(tracer: opentelemetry_sdk::trace::Tracer) -> Result<()> {
    ...
    // **always fails** when Opentelementry already called `log4rs::init_*` — returning Err even though
    ...
    if tracing::dispatcher::set_global_default(dispatch).is_err() {
        log::warn!(
            "opentelementry: global tracing subscriber already set (second Opentelementry::build in this process); \
             #[instrument] OTLP export applies only to the first Opentelementry instance"
        );
    }
...
/// Opentelementry tracing for manual span management.
///
...
///
/// ```no_run
/// use opentelementry::tracing::OpentelementryTracing;
///
/// // Construct from an existing telemetry tracer (or `None` when tracing is disabled).
/// let tracing = OpentelementryTracing::new(None);
///
/// let mut span = tracing.start_span("my_operation");
/// span.set_attribute("key", "value");
/// span.end();
/// ```
pub struct OpentelementryTracing {
    tracer: Option<opentelemetry_sdk::trace::Tracer>,
}

impl OpentelementryTracing {
    /// Creates a new OpentelementryTracing instance from an existing telemetry tracer.
