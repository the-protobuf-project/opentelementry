...
echo "=== Pulse Telemetry EKS Deployment ==="
...
kubectl -n pulse rollout status deployment/grafana --timeout=300s
kubectl -n pulse rollout status deployment/prometheus --timeout=300s
kubectl -n pulse rollout status deployment/otel-collector --timeout=300s
