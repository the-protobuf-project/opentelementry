...
echo "=== Pulse Telemetry EC2 Re-deployment ==="
...
ssh -i "$SSH_KEY" ec2-user@"$PUBLIC_IP" "cd /opt/pulse && sudo docker-compose -f docker-compose.prod.yaml down && sudo docker-compose -f docker-compose.prod.yaml up -d --build"
