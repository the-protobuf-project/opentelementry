...
// Priority: PULSE_CONFIG_PATH env var > pulse.toml > pulse.yaml > .config/pulse.toml > etc.
func discoverConfigPath() string {
	// Check PULSE_CONFIG_PATH environment variable first
	if envPath := os.Getenv("PULSE_CONFIG_PATH"); envPath != "" {
...
// Environment variables override file values. Env vars should be prefixed with PULSE_
// and use underscores for nesting (e.g., PULSE_TELEMETRY_OTLP_HOST).
func LoadConfig(configPath string) (*PulseOptions, *ServiceOptions, error) {
	...
	// Load environment variables with PULSE_ prefix
	// PULSE_TELEMETRY_OTLP_HOST -> telemetry.otlp.host
	if err := k.Load(env.Provider("PULSE_", ".", func(s string) string {
		return strings.ReplaceAll(
			strings.ToLower(strings.TrimPrefix(s, "PULSE_")),
			"_", ".")
	}), nil); err != nil {
	...
	var pulseOpts PulseOptions
	if err := k.UnmarshalWithConf("", &pulseOpts, unmarshalConf); err != nil {
		return nil, nil, fmt.Errorf("error unmarshalling pulse options: %w", err)
	}
	...
	return &pulseOpts, &serviceOpts, nil
}

// LoadConfigWithDefaults loads configuration and merges with defaults.
// If configPath is empty, auto-discovers config from:
//   - PULSE_CONFIG_PATH env var
//   - pulse.toml, pulse.yaml, pulse.json in current directory
//   - .config/pulse.toml, .config/pulse.yaml, .config/pulse.json
//
// Priority: defaults < config file < environment variables
func LoadConfigWithDefaults(configPath string) (*PulseOptions, *ServiceOptions, error) {
	// Start with defaults
	pulseOpts := Default()
...
func MustLoadConfig(configPath string) (*PulseOptions, *ServiceOptions) {
	pulseOpts, serviceOpts, err := LoadConfigWithDefaults(configPath)
	if err != nil {
		panic(err)
	}
	return pulseOpts, serviceOpts
}
