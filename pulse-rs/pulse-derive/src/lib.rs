//! Procedural macros for Pulse.
//!
//! This crate provides derive macros and attribute macros for the Pulse library.
//!
//! # Derive Macros
//!
...
pub fn derive_metrics(input: TokenStream) -> TokenStream {
            ...
            for attr in &field.attrs {
                ...
                if let (Some(name), Some(mtype)) = (metric_name, metric_type) {
                    let type_expr = match mtype.as_str() {
                        "counter" => quote! { pulse::metrics::MetricType::Counter },
                        "histogram" => quote! { pulse::metrics::MetricType::Histogram },
                        "gauge" => quote! { pulse::metrics::MetricType::Gauge },
                        _ => continue,
                    };
    ...
    let expanded = quote! {
        impl pulse::metrics::RecordMetrics for #name {
            fn metric_fields(&self) -> Vec<pulse::metrics::MetricField> {
                vec![
                    #(#metric_fields),*
                ]
            }
